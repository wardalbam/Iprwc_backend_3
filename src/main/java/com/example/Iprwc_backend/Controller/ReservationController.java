...
public class ReservationController {
    ...
    @PostMapping("/create")
public ResponseEntity<?> createBulkReservations(@RequestBody CreateBulkReservationsRequestDTO requestDTO, HttpServletRequest request) {
...
// delete by id admin
    ...
    public ResponseEntity<?> deleteReservationByIdUser(@PathVariable Long id, HttpServletRequest request) {
        Principal principal = request.getUserPrincipal();
        User user = userService.getUser(principal.getName());
        if( user.getRoles().contains(roleRepo.findByName("ROLE_USER") ) || user.getRoles().contains(roleRepo.findByName("ROLE_ADMIN") )){
            try {
                reservationService.deleteReservationById(id);
                return new ResponseEntity<>(HttpStatus.OK);
            } catch (Exception e) {
